        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                <div class="ibox-content">
                    <form action="/ERMS/Item/edit/" method="post" class="form-horizontal">
                        <input data-val="true" data-val-required="Id 字段是必需的。" id="Id" name="id" type="hidden" value="${item.id}" />
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" data-val="true" data-val-number="字段 所属频道 必须是一个数字。" data-val-required="所属频道不能为空！" id="ChannelId" name="channelId" onchange="v.search()">
                                    <option value="1" <c:if test="${(!empty item.ownerChannel) and (item.ownerChannel.id eq 1)}">selected</c:if>>新闻</option>
                                    <option value="2" <c:if test="${(!empty item.ownerChannel) and (item.ownerChannel.id eq 2)}">selected</c:if>>论文</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <select id="CategoryId" name="categoryId" class="form-control">
                                    ...
                                    <option v-for="(row,index) in data" :value="row.id" :selected="row.id==(${item.categoryId})?'selected':''" :style="row.parentId==0?'':'padding-left:20px;'">{{row.name}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" data-val="true" data-val-required="审核状态 字段是必需的。" id="Status" name="status">
                                    ...
                                    <option value="1" <c:if test="${item.status.code eq 1}">selected</c:if>>已通过</option>
                                    <option value="0" <c:if test="${item.status.code eq 0}">selected</c:if>>未通过</option>
                                </select>
                            </div>
                        </div>
        ...
        var v = app.vue({
            ...
            methods: {
                ...
                show_metadata_field_form() {
                    var url = "/ERMS/Item/ShowMetadataFieldForm?ChannelId=" + $("#ChannelId").val() + "&CategoryId=" + $("#CategoryId").val() + "&ItemId=${item.id}";
