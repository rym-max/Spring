...
<t:_PopupLayout>
    <form action="/ERMS/MetadataFieldRegistry/edit" method="post" class="form-horizontal">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-4">
                <select data-val="true" data-val-number="字段 元数据注册 必须是一个数字。" data-val-required="元数据注册 字段是必需的。" id="MetadataSchemaId" name="metadataSchemaId" class="form-control">
                    ...
                    <option  v-if="option.id==MetadataSchemaId" v-bind:value="option.id" selected>{{option.name}}</option>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-2">
                <input <c:if test="${(!empty mdfr.isRequired) && mdfr.isRequired}">checked="checked"</c:if> class="form-control i-checks" data-val="true" data-val-required="是否必填 字段是必需的。" id="IsRequired" name="isRequired" type="checkbox" value="true" />
                <input name="isRequired" type="hidden" value="false" />
            </div>
            <label class="col-sm-2 control-label" for="IsSearch">参与检索</label>
            <div class="col-sm-2">
                <input <c:if test="${(!empty mdfr.isSearch) && mdfr.isSearch}">checked="checked"</c:if> class="form-control i-checks" data-val="true" data-val-required="参与检索 字段是必需的。" id="IsSearch" name="isSearch" type="checkbox" value="true" />
                <input name="isSearch" type="hidden" value="false" />
            </div>
            <label class="col-sm-2 control-label" for="IsFullSearch">参与全文检索</label>
            <div class="col-sm-2">
                <input <c:if test="${(!empty mdfr.isFullSearch) && mdfr.isFullSearch}">checked="checked"</c:if> class="form-control i-checks" data-val="true" data-val-required="参与全文检索 字段是必需的。" id="IsFullSearch" name="isFullSearch" type="checkbox" value="true" />
        ...
        <div class="form-group">
            ...
            <div class="col-sm-2">
                <input <c:if test="${(!empty mdfr.isCluster) && mdfr.isCluster}">checked="checked"</c:if> class="form-control i-checks" data-val="true" data-val-required="可聚类 字段是必需的。" id="IsCluster" name="isCluster" type="checkbox" value="true" />
                <input name="isCluster" type="hidden" value="false" />
            </div>
            <label class="col-sm-2 control-label" for="IsSort">可排序</label>
            <div class="col-sm-2">
                <input <c:if test="${(!empty mdfr.isSort) && mdfr.isSort}">checked="checked"</c:if> class="form-control i-checks" data-val="true" data-val-required="可排序 字段是必需的。" id="IsSort" name="isSort" type="checkbox" value="true" />
                <input name="isSort" type="hidden" value="false" />
            </div>
            <label class="col-sm-2 control-label" for="IsMultiple">可多值</label>
            <div class="col-sm-2">
                <input <c:if test="${(!empty mdfr.isMultiple) && mdfr.isMultiple}">checked="checked"</c:if> class="form-control i-checks" data-val="true" data-val-required="可多值 字段是必需的。" id="IsMultiple" name="isMultiple" type="checkbox" value="true" />
...
<script>
    var v = app.vue({
        data: {
            metadataSchemaRegistrys: {},
            MetadataSchemaId:<c:if test="${!empty mdfr.ownerMetadataSchemaRegistry}" >${mdfr.ownerMetadataSchemaRegistry.id}</c:if><c:if test="${empty mdfr.ownerMetadataSchemaRegistry}" >0</c:if>
        },
        loading: true,
        methods: {
