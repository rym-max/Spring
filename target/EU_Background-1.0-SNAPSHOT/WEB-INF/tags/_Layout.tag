    ...
    <nav class="navbar-default navbar-static-side" role="navigation">
        <div class="sidebar-collapse">
            <ul class="nav metismenu" id="side-menu">
                ...
                <li <c:if test="${location eq 'HOME'}">class="active"</c:if>>
                    <a data-pjax href="/Home/index.html"><i class="fa fa-desktop"></i> 控制台</a>
                </li>
                <sec:authorize access="hasRole('ROLE_ADMIN')" >
                <li <c:if test="${location eq 'ADMINITRATOR'}">class="active"</c:if>>
                    <a data-pjax href="/Administrator/index.html">
                    <i class="fa fa-users"></i>
                    <span class="nav-label">系统管理员管理</span>
                    </a>
                </li>
                <%--<li <c:if test="${location eq 'USER'}">class="active"</c:if>>--%>
                ...
                <li <c:if test="${location eq 'REGION'}">class="active"</c:if>>
                    <a data-pjax href="/ERMS/Region/index.html">
                        <i class="fa fa-location-arrow"></i> 区域管理 <%--<span class="label label-primary pull-right">100</span>--%>
                    </a>
                </li>
                <li <c:if test="${location eq 'CHANNEL'}">class="active"</c:if>>
                    <a data-pjax href="/ERMS/Channel/index.html">
                    <i class="fa fa-table"></i> 栏目管理
                    </a>
                </li>
                <li <c:if test="${location eq 'CHANNELMETA'}">class="active"</c:if>>
                    <a data-pjax href="/ERMS/Channel/metadata.html">
                    <i class="fa fa-cog"></i> 栏目元数据设置
                    </a>
                </li>
                <li <c:if test="${location eq 'CATEGORY'}">class="active"</c:if>>
                    <a data-pjax href="/ERMS/Category/index.html">
                    <i class="fa fa-th"></i> 分类管理
                    </a>
                </li>
                <li <c:if test="${location eq 'CATEGORYMETA'}">class="active"</c:if>>
                    <a data-pjax href="/ERMS/Category/metadata.html">
                    <i class="fa fa-cog"></i> 分类元数据设置
                    </a>
                </li>
                <li <c:if test="${location eq 'SCHEMA'}">class="active"</c:if>>
                    <a data-pjax href="/ERMS/MetadataSchemaRegistry/index.html">
                    <i class="fa fa-th-large"></i> 元数据类型管理
                    </a>
                </li>
                <li <c:if test="${location eq 'FIELD'}">class="active"</c:if>>
                ...
                <%--<li <c:if test="${location eq 'ITEM'}">class="active"</c:if>>--%>
                    <%--<a data-pjax href="/ERMS/Item/index.html">--%>
                    <%--<i class="fa fa-file-pdf-o"></i> 文献管理 &lt;%&ndash;<span class="label label-primary pull-right">100</span>&ndash;%&gt;--%>
                    <%--</a>--%>
                <%--</li>--%>
                <li <c:if test="${location eq 'ALL'}">class="active"</c:if>>
                ...
                <%--</sec:authorize>--%>
                <sec:authorize access="hasAnyRole('ROLE_ADMIN','ROLE_ARTICLE')">
                <li <c:if test="${location eq 'SPIDERCONFIG'}">class="active"</c:if>>
                    <a data-pjax href="/Spider/SpiderConfig/index.html">
                        <i class="fa fa-bug"></i> 爬虫配置 <%--<span class="label label-primary pull-right">100</span>--%>
                    </a>
                </li>
                <li <c:if test="${location eq 'SPIDERITEM'}">class="active"</c:if>>
                    <a data-pjax href="/Spider/SpiderItem/index.html">
                        <i class="fa fa-bug"></i> 爬虫管理 <%--<span class="label label-primary pull-right">100</span>--%>
                    </a>
                </li>
                <li <c:if test="${location eq 'SOLR'}">class="active"</c:if>>
                    <a data-pjax href="/Solr/index.html">
                    <i class="fa fa-th-list"></i> 创建索引
                    </a>
                </li>
                <li <c:if test="${location eq 'JOURNAL'}">class="active"</c:if>>
                    <a data-pjax href="/Book/Journal/index.html">
                    <i class="fa fa-th-list"></i> 期刊管理
                    </a>
                </li>
                <li <c:if test="${location eq 'ACCESSLOG'}">class="active"</c:if>>
                    <a data-pjax href="/Log/AccessLog/index.html">
                    <i class="fa fa-area-chart"></i> 访问统计
                    </a>
                </li>
                <li <c:if test="${location eq 'SPIDERLOG'}">class="active"</c:if>>
                ...
                <ul class="nav nav-second-level collapse">
                    ...
                    <li><a data-pjax href="/@Url.Action("Index","SpiderItem",new { area = "ERMS" })">文献管理 <span class="label label-primary pull-right">100</span></a></li>
                </ul>
                </li>--%>
            </ul>
