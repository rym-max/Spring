        ...
        var v = app.vue({
            ...
            methods: {
                ...
                search(){
                    _self = this;
                    if($("#SpiderId").val()>0) {
                        http.post('/Log/SpiderLog/Search'+'?spider='+$("#SpiderId").val()+'&times='+$('#Times')).then(function (result) {
                            if (result.success) {
                ...
                searchUserCount(){
                    _self = this;
                    if($("#SpiderId").val()>0) {
                        http.post('/Log/Spider/SearchUserCount?spider=' + $('#SpiderId') + '&action=' + $('#Action')).then(
                            function (result) {
                                if (result.success) {
                            ...
                            <div class="col-lg-12">
                                ...
                                <div class="row pull-right" style="margin-bottom:20px;">
                                    <div class="col-lg-12">
                                        <form role="form" class="form-inline">
                                            <div class="input-group">
                                                <select id="Action" name="action" onchange="v.searchAction();" class="form-control">
                                                    <option value="ALL" selected>ALL</option>
                                                    <c:forEach items="actionEnums" var="item">
                                ...
                                <div class="col-lg-12">
                                    <table class="table table-striped table-bordered table-hover">
                                        ...
                                        <tbody v-cloak>
                                        <tr  v-for="(item,index) in usercount">
                                            <td>{{item.user}}</td>
                                            <td>{{item.action}}</td>
                                            <td>{{item.spider}}</td>
                                            <td>{{item.count}}</td>
